...
if ($videoIdRecommended !=="no recommended defined") {
 
//limit to active user--
   $sqlSearch=mysqli_prepare($con,"SELECT videoID,title,source,views,description,email,dateUpload,category FROM videos where videoID=?");
//"s" datatype order. one s string 
//@param s string
mysqli_stmt_bind_param($sqlSearch,"s",$videoIdRecommended);
